...
// scripts/run-kyc-and-deploy-mint.js
// const hre = require("hardhat");

// const main = async () => {
//   const [issuer, user] = await hre.ethers.getSigners();

//   // 1. Attach to deployed KYCVerifier
//   const KYC_ABI = [
//   {
//     "inputs": [
//       {
...
//   }
// ];
//   const KYC_ADDRESS = "0x16B3574b38AE3653e6768b75344AE2E49D64ED0b";
//   const kyc = await hre.ethers.getContractAt(KYC_ABI, KYC_ADDRESS, issuer);

//   // 2. Build & sign EIP-712 KYC payload
//   const { chainId } = await hre.ethers.provider.getNetwork();
//   const domain = {
//     name: "Gbese KYC",
//     version: "1",
//     chainId: Number(chainId),
//     verifyingContract: KYC_ADDRESS,
//   };
//   const types = {
//     KYC: [
//       { name: "user", type: "address" },
...
//   const signature = await hre.ethers.provider.send(
//     "eth_signTypedData_v4",
//     [issuer.address, payload]
//   );
//   console.log("KYC signature:", signature);

//   // 3. User calls verifyKYC
//   let tx = await kyc.connect(user).verifyKYC(user.address, issuedAt, signature);
//   console.log("verifyKYC tx hash:", tx.hash);
//   await tx.wait();

//   // 4. Deploy GbeseToken now that KYC is done
//   const GbeseFactory = await hre.ethers.getContractFactory("GbeseToken");
//   const gbese = await GbeseFactory.deploy(KYC_ADDRESS);
//   await gbese.waitForDeployment();
//   console.log("GbeseToken deployed to:", gbese.target);

//   // 5. Mint 10 GBT to the verified user
//   const mintAmount = hre.ethers.parseUnits("10", 18);
//   tx = await gbese.connect(issuer).mint(user.address, mintAmount);
